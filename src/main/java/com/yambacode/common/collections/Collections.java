...
public class Collections {
    ...
    public static <T> T[] copiesOf(T object, int count) {
        List<T> collect = IntStream.range(1, count).mapToObj(x -> object).collect(toList());
    ...
    public static List<List<Character>> copyOfNested(List<List<Character>> accu) {
        return accu.stream().map(list -> copyOf(list)).collect(toList());
    }

    public static List<Character> copyOf(List<Character> list) {
        return list.stream().map(c -> Character.valueOf(c)).collect(toList());
    }
